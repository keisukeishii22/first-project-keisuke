...
DELIVERABLES = [
    ("note", "note記事下書き",
     ...
     "テーマに[検索型]が含まれる場合は「SEOブロック」も追加してください:"
     "①想定検索キーワード(主1+関連2〜3)②タイトル案は主キーワードを前方配置③H2見出しへのキーワード配置"
     "④FAQ 3問(検索者が次に調べる質問をQ&A形式で本文末尾に)⑤noteハッシュタグ案5個。"
     "導入の200字以内で検索意図への答えを先出ししてください。"),
    ("reel", "リール台本2本",
...
def update_progress_memo(week_num: int, today: str) -> None:
    ...
    new_row = f"| 第{week_num}週 | {today} | 素材待ちドラフト生成済み | 未公開 | インタビュー回答到着後に実体験を肉付け |\n"
